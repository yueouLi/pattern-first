#!/usr/bin/env bash
#
# pattern-first / uninstall.sh
#
# Removes the 14 pattern-first skills from Claude Code and/or Codex skill dirs.
#
...
remove_skills() {
  ...
  echo ""
  echo "Removing pattern-first from $label:"
...
echo ""
echo "To re-install: bash install.sh [--codex|--all] (from pattern-first source root)"
echo ""
